// https://github.com/turbomaze/JS-Fourier-Image-Analysis/blob/master/js/fourier.js

export const Fourier = function () {
  /******************
   * work functions */
  ...
  function invFFT(transform, sig) {
    ...
    for (const ai = 0; ai < sig.length; ai++) {
  ...
  function shiftFFT(transform, dims) {
    return flipRightHalf(
      halfShiftFFT(halfShiftFFT(transform, dims), dims),
      dims
    );
  }
  ...
  function unshiftFFT(transform, dims) {
    return halfShiftFFT(
      halfShiftFFT(flipRightHalf(transform, dims), dims),
      dims
    );
  }
  ...
  function halfShiftFFT(transform, dims) {
    ...
    const N = dims[1];
    const M = dims[0];
    for (const n = 0, vOff = N / 2; n < N; n++) {
      for (const m = 0; m < M / 2; m++) {
